...
class MetallistProApp:
    ...
    def draw_geometry_sketch(self):
        ...
        if gtype == "Круг / Сегмент":
            ...
            p = []
            for i in range(6):
                p.extend([cx + 50*math.cos(math.radians(i*60)), cy + 50*math.sin(math.radians(i*60))])
    ...
    def process_geometry(self):
        ...
        try:
            if gtype == "Круг / Сегмент":
                R = float(self.entries["Радиус R, мм"].get())
                a = float(self.entries["Угол альфа, град"].get())
                res += f"Площадь круга: {math.pi*(R**2):.2f} мм²\n"
                res += f"Площадь сегмента: {0.5*(R**2)*(math.radians(a)-math.sin(math.radians(a))):.2f} мм²\n"
            elif gtype == "Труба (Кольцо)":
                D = float(self.entries["Внешний диаметр D, мм"].get())
                s = float(self.entries["Толщина стенки s, мм"].get())
                ...
                S = float(self.entries["Размер под ключ S, мм"].get())
                L = float(self.entries["Длина L, мм"].get())
                res += f"Площадь сечения: {(math.sqrt(3)/2)*(S**2):.2f} мм²\n"
                res += f"Вес заготовки: {((math.sqrt(3)/2)*(S**2)*L*rho)/1000000:.3f} кг\n"
    ...
    def rebuild_sortament_manual_inputs(self):
        # Полная очистка динамического фрейма ввода
        ...
        if "Лист" in prof:
            # Для листа: показываем ширину, длину и толщину
            ...
            e_w = tk.Entry(self.sort_manual_frame, width=10, bg="#fff2cc", justify="center")
            e_w.insert(0, "1500")
            ...
            e_l = tk.Entry(self.sort_manual_frame, width=10, bg="#fff2cc", justify="center")
            e_l.insert(0, "6000")
            ...
            e_t = tk.Entry(self.sort_manual_frame, width=8, bg="#fff2cc", justify="center")
            e_t.insert(0, "4")
            e_t.grid(row=0, column=5, padx=5, pady=6, sticky="w")
            self.sort_dyn_widgets["Толщина"] = e_t
        else:
            # Для труб, двутавров и швеллеров
    ...
    def proc_detali_calc(self):
        ...
        if "Фланец" in t:
            ...
            try:
                ...
                messagebox.showerror("Ошибка", "Проверьте числовой ввод Диаметра и Стенки отвода!")
                return
    ...
    def init_metiz_tab(self):
        ...
        # Конфигурация жесткой двухрядной сетки
        ...
        ttk.Label(g1, text="n штук").grid(row=0, column=0, padx=2)
        ttk.Label(g1, text="диаметр").grid(row=0, column=1, padx=2)
        ttk.Label(g1, text="Длина").grid(row=0, column=2, padx=2)
        ttk.Label(g1, text="Масса ед.").grid(row=0, column=3, padx=2)
        ttk.Label(g1, text="Общая масса, кг").grid(row=0, column=4, padx=2)
        
        self.b_pcs = tk.Entry(g1, width=7, bg="#fff2cc", justify="center")
        self.b_pcs.insert(0, "8")
        self.b_pcs.grid(row=1, column=0, pady=2, padx=2)
        
        self.b_d = ttk.Combobox(g1, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=7, state="readonly")
        self.b_d.set("М16")
        self.b_d.grid(row=1, column=1, pady=2, padx=2)
        
        self.b_l = ttk.Combobox(g1, values=[str(x) for x in range(40, 161, 10)] + [str(x) for x in range(180, 301, 20)], width=6, state="readonly")
        self.b_l.set("50")
        ...
        ttk.Label(g1_rev, text="кг").grid(row=0, column=0, padx=2)
        ttk.Label(g1_rev, text="диаметр").grid(row=0, column=1, padx=2)
        ttk.Label(g1_rev, text="Длина").grid(row=0, column=2, padx=2)
        ttk.Label(g1_rev, text="Кол-во шт.").grid(row=0, column=3, padx=2)
        ...
        self.b_d_rev = ttk.Combobox(g1_rev, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=7, state="readonly")
        self.b_d_rev.set("М16")
        self.b_d_rev.grid(row=1, column=1, pady=2, padx=2)
        
        self.b_l_rev = ttk.Combobox(g1_rev, values=[str(x) for x in range(40, 161, 10)] + [str(x) for x in range(180, 301, 20)], width=6, state="readonly")
        self.b_l_rev.set("50")
        ...
        ttk.Label(g2, text="n штук").grid(row=0, column=0, padx=2)
        ttk.Label(g2, text="диаметр").grid(row=0, column=1, padx=2)
        ttk.Label(g2, text="Масса ед.").grid(row=0, column=2, padx=2)
        ttk.Label(g2, text="Общая масса, кг").grid(row=0, column=3, padx=2)
        
        self.n_pcs = tk.Entry(g2, width=7, bg="#fff2cc", justify="center")
        self.n_pcs.insert(0, "8")
        self.n_pcs.grid(row=1, column=0, pady=2, padx=2)
        
        self.n_d = ttk.Combobox(g2, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=8, state="readonly")
        self.n_d.set("М16")
        ...
        ttk.Label(g2_rev, text="кг").grid(row=0, column=0, padx=2)
        ttk.Label(g2_rev, text="диаметр").grid(row=0, column=1, padx=2)
        ttk.Label(g2_rev, text="Кол-во шт.").grid(row=0, column=2, padx=2)
        ...
        self.n_d_rev = ttk.Combobox(g2_rev, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=8, state="readonly")
        ...
        ttk.Label(g3, text="n штук").grid(row=0, column=0, padx=2)
        ttk.Label(g3, text="тип (размер)").grid(row=0, column=1, padx=2)
        ttk.Label(g3, text="Масса ед.").grid(row=0, column=2, padx=2)
        ttk.Label(g3, text="Общая масса, кг").grid(row=0, column=3, padx=2)
        ...
        self.w_size = ttk.Combobox(g3, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=9, state="readonly")
        self.w_size.set("М16")
        ...
        ttk.Label(g3_rev, text="кг").grid(row=0, column=0, padx=2)
        ttk.Label(g3_rev, text="диаметр").grid(row=0, column=1, padx=2)
        ttk.Label(g3_rev, text="Кол-во шт.").grid(row=0, column=2, padx=2)
        ...
        self.w_size_rev = ttk.Combobox(g3_rev, values=["М10", "М12", "М16", "М20", "М24", "М30", "М36", "М42", "М48"], width=9, state="readonly")
        ...
        ttk.Label(g4, text="n штук").grid(row=0, column=0, padx=2)
        ttk.Label(g4, text="dxL (размер)").grid(row=0, column=1, padx=2)
        ttk.Label(g4, text="Масса ед.").grid(row=0, column=2, padx=2)
        ttk.Label(g4, text="Общая масса, кг").grid(row=0, column=3, padx=2)
        ...
        self.s_size = ttk.Combobox(g4, values=["М12х60", "М16х90", "М20x110", "М24x130", "М30x150", "М36x180", "М42x220", "М48x260", "2.5x16", "4.2x25"], width=12, state="readonly")
        ...
        ttk.Label(g4_rev, text="кг").grid(row=0, column=0, padx=2)
        ttk.Label(g4_rev, text="dxL (размер)").grid(row=0, column=1, padx=2)
        ttk.Label(g4_rev, text="Кол-во шт.").grid(row=0, column=2, padx=2)
        ...
        self.s_size_rev = ttk.Combobox(g4_rev, values=["М12х60", "М16х90", "М20x110", "М24x130", "М30x150", "М36x180", "М42x220", "М48x260", "2.5x16", "4.2x25"], width=12, state="readonly")
    ...
    def proc_all_metiz_logic(self):
        ...
        try:
            ...
            w_b_one = float(self.b_one.get()) if self.b_one.get() else 0
        ...
        try:
            ...
            w_n_one = float(self.n_one.get()) if self.n_one.get() else 0
        ...
        try:
            ...
            w_w_one = float(self.w_one.get()) if self.w_one.get() else 0
        ...
        try:
            ...
            w_s_one = float(self.s_one.get()) if self.s_one.get() else 0
    ...
    def init_welding_tab(self):
        ...
        self.out_e = ttk.Entry(f_r1, width=10, font=("Consolas", 10, "bold"), justify="center")
        ...
        self.out_current = ttk.Entry(f_r1, width=15, font=("Consolas", 10, "bold"), justify="center")
        self.out_current.pack(side="right", padx=15)
        
        f_r2 = ttk.Frame(res_box)
        f_r2.pack(fill="x", pady=4)
        self.out_el_mass = ttk.Entry(f_r2, width=10, font=("Consolas", 10, "bold"), justify="center")
    ...
    def update_welding_marks_list(self, event=None):
        ...
        if "Углеродистая" in cat:
            marks = ["ТМУ-21У", "ОЗС-4", "ОЗС-6", "ОЗС-12", "АНО-4", "УОНИ-13/45", "УОНИ-13/55", "МР-3"]
    ...
    def proc_welding_current_and_rates(self, event=None):
        ...
        current_map = {
            "УОНИ-13/45": {"2.0": {"Нижнее": "40-60", "Вертикальное": "35-55", "Потолочное": "35-55"}, "2.5": {"Нижнее": "50-75", "Вертикальное": "40-65", "Потолочное": "40-65"}, "3.0": {"Нижнее": "80-100", "Вертикальное": "70-90", "Потолочное": "70-90"}, "4.0": {"Нижнее": "130-150", "Вертикальное": "130-140", "Потолочное": "130-140"}},
            "УОНИ-13/55": {"2.0": {"Нижнее": "40-60", "Вертикальное": "35-55", "Потолочное": "35-55"}, "2.5": {"Нижнее": "50-75", "Вертикальное": "40-65", "Потолочное": "40-65"}, "3.0": {"Нижнее": "80-100", "Вертикальное": "70-90", "Потолочное": "70-90"}, "4.0": {"Нижнее": "130-160", "Вертикальное": "130-140", "Потолочное": "130-140"}},
            "АНО-4": {"3.0": {"Нижнее": "100-140", "Вертикальное": "90-110", "Потолочное": "100-120"}, "4.0": {"Нижнее": "170-210", "Вертикальное": "140-150", "Потолочное": "140-170"}},
            "МР-3": {"3.0": {"Нижнее": "140-180", "Вертикальное": "120-160", "Потолочное": "120-160"}, "4.0": {"Нижнее": "160-200", "Вертикальное": "140-180", "Потолочное": "140-180"}},
    ...
    def process_gost_welding_calculations(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Проверьте числовые параметры шва!\n{str(e)}")
    ...
    def init_designation_tab(self):
        ...
        for z_txt, _ in znaks:
            ttk.Radiobutton(f_r, text=z_txt, variable=self.znak_var, value=z_txt, command=self.update_gost_text_info).pack(side="left", padx=10)
        ...
        self.gost_desc_txt = tk.Text(f_gosts, bg="#f8f9fa", font=("Segoe UI", 10), width=45)
