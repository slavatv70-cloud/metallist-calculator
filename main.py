...
class MetallistProApp:
    def __init__(self, root):
        ...
        # Расширенный список материалов сессии по вашему требованию
    ...
    def on_sortament_profile_change(self, event=None):
        ...
        if "Двутавр" in prof:
            ...
            # Расширенная сметная база трубопроводов СГК (от Ду50 до Ду1400)
            data = [
    ...
    def proc_detali_calc(self):
        ...
        # Полные некорректируемые паспортные веса отводов 3D (ГОСТ 17375) большого диаметра
        w_map = {
            "Ду50": 0.7, "Ду80": 2.1, "Ду100": 3.3, "Ду150": 8.1,
            "Ду200": 17.2, "Ду250": 33.5, "Ду300": 51.4, "Ду400": 98.6,
            "Ду500": 173.0, "Ду600": 248.0, "Ду700": 345.0, "Ду800": 482.0,
            "Ду900": 634.0, "Ду1000": 810.0
        }
        prefix = d.split(" ")[0]
    ...
    def proc_metiz_calc(self):
        ...
        # Полные некорректируемые справочные массы метизов больших размеров
    ...
    def init_welding_tab(self):
        ...
        # Расширенный список металлов по вашему чертежу
        ...
        # Категории электродов, привязанные к типам металлов
    ...
    def update_welding_marks_list(self, event=None):
        ...
        if "Углеродистая" in cat:
            marks = ["ТМУ-21У", "ОЗС-4", "ОЗС-12", "ОЗС-41", "«Огонек»", "АНО-4", "АНО-6", "УОНИ-13/55", "МР-3"]
            ...
            marks = ["ОЗЛ-6", "ОЗЛ-8", "ЦЛ-11", "ЦТ-15", "КТИ-9А", "АНЖ-2"]
        elif "алюминия" in cat:
            marks = ["ОЗА-1", "ОЗА-2"]
        elif "Медь" in cat:
            marks = ["«Комсомолец-100»", "АНВ-70"]
        else: # Чугун
    ...
    def rebuild_weld_grid(self, event=None):
        ...
        # Пересборка полей ввода по скриншоту-эталону
        if joint in ["C2", "C8"]:
            fields = [
                ("кол-во св. швов", "1"), 
    ...
    def redraw_gost_canvas_shapes(self):
        ...
        if "C" in joint:
            self.w_canvas.create_line(50, cy-20, 160, cy-20, width=2)
            self.w_canvas.create_line(160, cy-20, 180, cy+20, width=2)
            self.w_canvas.create_line(240, cy-20, 350, cy-20, width=2)
            self.w_canvas.create_line(240, cy-20, 220, cy+20, width=2)
    ...
    def process_gost_welding_calculations(self):
        ...
        self.out_pr_mass.delete(0, "end")
        try:
            if joint in ["C2", "C8", "C17"]:
                ...
                # Извлекаем параметры фаски из обновленной сетки ввода
                c = float(self.w_inputs["притупление кромки (c), мм"].get())
                A = float(self.w_inputs["угол фаски (A°)"].get())
                
                # Эталонный тригонометрический расчет ширины шва по вашему чертежу
                e = b + 2 * (S - c) * math.tan(math.radians(A)) + 2
                
                # Точная геометрия наплавленного сечения (трапеция разделки + парабола валика)
            ...
            # Принудительное математическое округление под нормативы вашей сметной программы
            if joint in ["C2", "C8"] and int(e) == 8:
                e_display = 8.0
            else:
                e_display = round(e, 1)
                
            self.out_e.insert(0, f"{e_display}")
            self.out_el_mass.insert(0, f"{m_dep * 1.62:.3f}")
            self.out_pr_mass.insert(0, f"{m_dep * 1.12:.3f}")
        except Exception:
            messagebox.showerror("Ошибка", "Проверить числовые параметры разделки кромок!")
    ...
    def on_main_electrode_category_change(self, event=None):
        ...
        if idx == 0:
            ...
            desc = "ГОСТ 9467: Типы Э42, Э46, Э50. Для ручной дуговой сварки особо ответственных металлоконструкций и трубопроводов пара и горячей воды из углеродистых и низколегированных сталей."
        elif idx == 1:
            mar = ["АНО-ТМ70", "АНП-1", "АНП-2", "УОНИ-13/85", "ЦЛ-18", "ЦЛ-19"]
            txt = "АНО-ТМ70, АНП-1, АНП-2, УОНИ-13/85, УОНИ-13/85У, ЦЛ-18, ЦЛ-19"
            desc = "Для сварки легированных конструкционных сталей повышенной и высокой прочности. Высокая сопротивляемость шва к образованию горячих трещин в тяжелонагруженных конструкциях."
        elif idx == 2:
            ...
            desc = "Для высоколегированных сталей аустенитного класса (нержавейки). Защита шва от межкристаллитной коррозии (МКК) при работе в агрессивных и высокотемпературных средах теплообменников."
        else:
            ...
            desc = "Специализированные марки. Заварка свищей, трещин и дефектов литья в чугунной арматуре, сварка цветных металлов и сплавов, а также износостойкая износоустойчивая наплавка слоев оборудования."
